...
## 7. International expansion — COLOMBIA SCORED; PARAGUAY DOCUMENTARY PILOT NOT SCORED

**Colombia/SECOP II (import 2026-09-22, indicators same day):** first international cohort. Three buyers announced before download (MEN national / Gobernación de Caldas departmental / Alcaldía Local de Usaquén municipal-local), 7,560 rows, 24-month signature window, raw pages under `data/colombia-secop2/raw/` (28 MB), extract 15 MB, coverage with `joinVerification` (100 % process/contract/supplier-name fields; 7,553/7,560 usable supplier IDs after excluding seven placeholders) and `counts.liquidation` (Si 1,542 / No 6,018). Spanish verbatim, COP only, no conversion. Licence CC BY-SA 4.0 verified.

**Colombian indicators (implemented 2026-09-22):** method `docs/score-colombia.md`, code branch in `getAssessmentSecop2` (`script.js`). Four checks, v3.0 framework, **jurisdiction-specific thresholds**: `secop2-plurality-award` (18 flat; justification ∈ {No existe pluralidad de oferentes, Urgencia manifiesta} on a direct-family modality — **the bare “Contratación directa” ≈82 % base never fires**), `secop2-repeated-plurality` (same buyer NIT + supplier document, ≥3 such awards → 18..60), `secop2-concentration` (buyer + contract type, ≥10 known, ≥80 % coverage, share ≥60 % → 12..40; **0 signals**, max observed share 31.3 %), `secop2-long-duration` (published free text parsed read-only, ≥36 months → 8..40). The four French offer/timetable/history checks are **out of scope with reasons** (no offers table, no chronology, no amendment history) so every row still shows exactly 8 checks. Cohort totals: **215 flagged / 7,345 zero / 0 not assessed / 71 partial**; fires 176 / 15 / 0 / 39. French datasets untouched (review counts identical: 277/404/124/0/0). Importer bug fixed en route: `liquidación` → `liquidaci_n` (was null on all rows; now tested by `tests/test_import_colombia_secop2.py`). Extract + coverage regenerated via `--offline`.

**Paraguay access validated (2026-09-22):** despite the Swagger’s global Bearer declaration, three data routes answered **HTTP 200 without a token** — `/ocds/record/ocds-03ad3f-365292-1` (full OCDS 1.1 record package), date-filtered `/search/processes` (one row), and `/parameters/parameters` (100 entries). Payload licence **CC BY 4.0**, publisher DNCP, policy `/datos/legal`. **Correct base path is `/datos/api/v3/doc` (Swagger `basePath`), not `/datos/api/v3`.** Search requires at least one filter (`page`/`items_per_page` are not enough). Probes: `python tools/check-international-access.py Paraguay` (label-substring filter). **Still open before any broader import or scoring:** quotas/exhaustiveness of national history, rate limits, and validation of local indicators (never a French/Colombian transplant). **Brazil** stays blocked (PNCP access not established). Ukraine/Moldova/TED-EU remain lighter options. Checks log: `data/international-access-checks.json`. No CPI-driven selection, ever.

**Paraguay pilot added after access validation:** one-buyer documentary cohort (Municipalidad de Fernando de la Mora, SICP 66), fixed call-publication window 2024-09-01 → 2025-09-01. 88 full OCID records retained from two search pages, yielding 84 explicitly award-linked contract entries (2 contract entries excluded); PYG only; 80 rows carry a `contractSigned`-typed document link, no published `dateSigned`. No Paraguayan scoring rules approved: 84 Not assessed. See `docs/paraguay-pilot.md`, `tools/import-paraguay-dncp.py`, `data/paraguay-dncp-coverage.json`, `tests/paraguay.cjs`. Older references below to Paraguay as only an access check describe the state before this pilot.

**Outcome-label safeguard:** optional `corruptionOutcome` is reserved for final adjudications with an official judgment, proof of finality and an exact contract link. The UI offers a separate filter; none of the currently provided French, Colombian or Paraguayan rows qualifies. A second, distinct press-lead filter marks only `crc-station-nuage`: September 2024 reporting of a search tied to Station Nuage, current investigation status unknown, no guilt inferred. Eight CRC audit findings remain audit findings, never corruption convictions or scoring inputs. Method: `docs/adjudicated-outcomes.md`; regression: `tests/outcomes.cjs`.
