...
function nationalSupplierIdentity(c) {
  ...
  return ['EDRPOU', 'RNOKPP', 'NIF', 'CUI', 'GB-FTS'].includes(identifierType) && typeof id === 'string' && id.trim() ? `${identifierType}:${id.trim()}` : null;
}
...
function validateContracts(data) {
  ...
  for (const c of data) {
    ...
    if (c.dataFamily != null && !['decp', 'boamp', 'audit', 'secop2', 'dncp', 'prozorro', 'ted', 'fts'].includes(c.dataFamily)) throw new Error(`${c.id} : invalid data family.`);
  ...
  function render() {
    ...
    pageRows.forEach((c, index) => {
      ...
      if (c.dataFamily === 'dncp') {
        ...
        const where = c.dataFamily === 'ted' ? 'TED eForms award notice' : c.dataFamily === 'fts' ? 'Find a Tender award notice' : 'Prozorro official record';
